...
**I think the x_star should be y_star, it is the true value of y, rather than x. **
...
```{r}
ggplot() +
  geom_point(data = data_1, mapping = aes(xx, yy)) +
...
Now, calculate the SSE: 
```{r}
sum((data_1$xx * 0.437 + 2.614 - data_1$x_star) ^ 2)
```

\[SSE_{Deming}=231.4674\]


```{r}
sum((data_1$xx * 0.343 + 3.022 - data_1$x_star) ^ 2)
```

\[SSE_{Regular}=232.5568. \]
...
For $D$, 
\[det(C)=-1\cdot\left|\begin{matrix}2&1\\3&2\end{matrix}\right|-1\cdot\left|\begin{matrix}1&1\\2&1\end{matrix}\right|=-1-(-1)=0.\]
...
```
```{r}
anova(lm(hba1c~fbg,data_2))
```
```{r}
anova(lm(hba1c~fbg+tg,data_2))
```

| Source of variation 	| df 	| Sum of Squares 	| MS 	| F-stat 	|
|:---:	|:---:	|:---:	|:---:	|:---:	|
| fbg 	| 1 	| 2796.47 	| 2796.47 	| 1682.4 	|
| fbg+tg 	| 2 	| 0.23 	| 0.23 	| 0.1399 	|
| tg 	| 1 	| 181.529 	| 181.529 	| 45.44 	|
| tg+fbg 	| 2 	| 2615.18 	| 1572.10 	|  	|
